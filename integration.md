...
- If `tenantVerification.status === null` → redirect to `/dashboard/verify-institution`
- If `tenantVerification.status === 'REJECTED'` → redirect to `/dashboard/verify-institution` (show reason)
...
4) If `tenantVerification.status === null`, route to your upload page (example: `/dashboard/verify-institution`)
