...
- Preview is optional: if the Group Leader wants to see the email first, they can preview; they can still send directly.
...
3. UI confirms success and refreshes state.
...
## Minimal TypeScript helpers (repo-aligned)
...
export async function previewProjectGroupInvitation(
  ...
  const res = await apiClient.post<ProjectGroupInvitationPreviewResult>(
