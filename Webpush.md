...
Current frontend implementation in this repo:

- Service worker: `public/push-sw.js`
- Push API helpers: `src/lib/api/push-notifications.ts`
- Push lifecycle helper: `src/lib/push-notifications.ts`
- Bell controls: `src/components/notifications/notification-bell.tsx`
- Settings controls: `src/components/dashboard/settings/push-notification-settings.tsx`
...
Use this checklist against the current frontend implementation in this repo:

1) Configure backend env vars and restart backend.
2) Confirm `GET /api/v1/notifications/push/vapid-public-key` returns a real public key.
3) Open frontend Settings -> `Notifications` tab, or use the notification bell controls.
4) Enable browser alerts if permission is still `default`.
5) Click `Prepare` under `Web Push`.
6) Confirm the status changes to `Active`.
7) Trigger a backend notification.
8) Confirm the push notification arrives when the tab is closed.
9) Click the push notification and confirm it opens the correct app route.
10) Click `Disable` under `Web Push` and confirm delivery stops.

Implementation notes for this repo:

- Frontend uses the shared axios client, so auth and tenant headers are handled automatically.
- The frontend now registers the browser subscription with the backend during setup.
- Unsubscribe removes the browser subscription and also calls the backend unsubscribe endpoint.
