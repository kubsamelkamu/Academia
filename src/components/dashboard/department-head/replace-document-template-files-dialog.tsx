...
export function ReplaceDocumentTemplateFilesDialog({
  ...
  const replaceMutation = useReplaceDocumentTemplateFiles(departmentId)
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to replace files")
    }
  }
  ...
  return (
    ...
    >
      ...
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>Replace all files</DialogTitle>
          <DialogDescription>
            This will replace all existing files for “{templateTitle}”. PDF/DOCX only. Max 10 files, 10MB each.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button type="button" onClick={submit} disabled={replaceMutation.isPending}>
